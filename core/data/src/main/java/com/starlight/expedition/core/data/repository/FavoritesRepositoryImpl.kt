...
class FavoritesRepositoryImpl(
    ...
    override fun observeFavoriteIds(): Flow<Set<String>> =
        context.starlightDataStore.data.map { preferences ->
            preferences[PreferencesKeys.FAVORITE_GAME_IDS] ?: SampleGameData.defaultFavoriteIds
        }

    override suspend fun setFavorite(gameId: String, favorite: Boolean) {
        context.starlightDataStore.edit { preferences ->
            val current = preferences[PreferencesKeys.FAVORITE_GAME_IDS] ?: SampleGameData.defaultFavoriteIds
